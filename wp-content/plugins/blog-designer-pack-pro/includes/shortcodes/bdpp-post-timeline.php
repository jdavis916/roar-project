<?php
/**
 * 'bdp_timeline' Post Timeline Shortcode
 * 
...
/**
 * Function to handle the `bdp_timeline` shortcode
 * 
 * @package Blog Designer Pack Pro
 * @since 1.0
 */
function bdpp_render_post_timeline( $atts, $content = null )  {
	...
	$shortcode_designs 				= bdpp_post_timeline_designs();
	...
	$atts['show_full_content']		= ( $atts['show_full_content'] == 'false' )	? false 								: true;
	...
	if ( $query->have_posts() ) {
			...
			bdpp_generate_style( $style_id, 'shortcode', 'grid', 'bdp_timeline', $atts );

			bdpp_get_template( 'timeline/loop-start.php', $atts );
			...
			while ( $query->have_posts() ) : $query->the_post();
	            ...
	            // Include Dsign File
				bdpp_get_template( "timeline/{$design}.php", $atts );

			endwhile;

			bdpp_get_template( 'timeline/loop-end.php', $atts );
...
add_shortcode( 'bdp_timeline', 'bdpp_render_post_timeline' );