<?php
/**
 * 'bdp_post_gridbox_slider' Post GridBox Slider Shortcode
 * 
...
/**
 * Function to handle the `bdp_post_gridbox_slider` shortcode
 * 
 * @package Blog Designer Pack Pro
 * @since 1.0
 */
function bdpp_render_post_gridbox_slider( $atts, $content = null )  {
	...
	$atts = shortcode_atts(array(
		...
		'custom_param_2'		=> '',
		), $atts, 'bdp_post_gridbox_slider');

	$shortcode_designs 				= bdpp_post_gridbox_slider_designs();
	...
	$atts['shortcode']				= 'bdp_post_gridbox_slider';
	...
	$atts['height']					= bdpp_clean_number( $atts['height'], 500 );
	...
	$post_type_arr			= array();
	$atts['count'] 			= 0;
	$atts['slider_conf']	= compact('loop', 'slide_margin', 'start_position', 'arrows', 'dots', 'autoplay', 'autoplay_interval', 'speed', 'autoplay_hover_pause', 'nav_text', 'lazyload', 'url_hash_listener', 'stage_padding', 'auto_height');
	...
	$args = apply_filters( 'bdpp_post_gridbox_slider_query_args', $args, $atts );
	...
	if ( $query->have_posts() ) {		
			...
			bdpp_generate_style( $style_id, 'shortcode', 'slider', 'bdp_post_gridbox_slider', $atts );
			
			bdpp_get_template( 'gridbox-slider/loop-start.php', $atts );
			...
			while ( $query->have_posts() ) : $query->the_post();
	            ...
	            // Include Dsign File
				bdpp_get_template( "gridbox-slider/{$design}.php", $atts );

			endwhile;

			bdpp_get_template( 'gridbox-slider/loop-end.php', $atts );
...
// Post GridBox Slider Shortcode
add_shortcode( 'bdp_post_gridbox_slider', 'bdpp_render_post_gridbox_slider' );