<?php
/**
 * 'bdp_masonry' Post Grid Shortcode
 * 
 ...
 * @since 1.0
 */
function bdpp_render_post_masonry( $atts, $content = null )  {
	...
	$shortcode_designs 				= bdpp_post_masonry_designs();
	...
	$atts['grid']					= !empty($atts['grid']) 					? $atts['grid'] 						: 1;
	...
	$atts['effect']					= bdpp_clean( $atts['effect'] );
	...
	if ( $query->have_posts() ) {		
			...
			bdpp_generate_style( $style_id, 'shortcode', 'grid', 'bdp_masonry', $atts );

			bdpp_get_template( 'masonry/loop-start.php', $atts );
			...
			while ( $query->have_posts() ) : $query->the_post();
	            ...
	            // Include Dsign File
				bdpp_get_template( "masonry/{$design}.php", $atts );

			endwhile;

			bdpp_get_template( 'masonry/loop-end.php', $atts );
...
add_shortcode( 'bdp_masonry', 'bdpp_render_post_masonry' );