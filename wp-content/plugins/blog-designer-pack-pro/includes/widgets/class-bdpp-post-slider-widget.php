...
class BDPP_Post_Slider_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Widget settings
		$widget_ops = array( 'classname' => 'bdpp-post-slider-widget', 'description' => __( 'Display posts in a slider view.', 'blog-designer-pack' ) );

		// Create the widget
		parent::__construct( 'bdpp-post-slider-widget', __( 'BDPP - Post Slider Widget', 'blog-designer-pack' ), $widget_ops );

		// Widgets defaults
		$this->defaults = array(
				'title' 				=> esc_html__( 'Post Slider', 'blog-designer-pack' ),
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['lazyload']				= ( !empty( $new_instance['lazyload'] ) ) 				? 1 : 0;
		$instance['autoplay_hover_pause']	= ( !empty( $new_instance['autoplay_hover_pause'] ) ) 	? 1 : 0;
		$instance['speed']					= bdpp_clean_number( $new_instance['speed'], 0 );
		$instance['autoplay_interval']		= bdpp_clean_number( $new_instance['autoplay_interval'], 5000 );
	...
	function form( $instance ) {
		...
		$widget_designs		= bdpp_post_slider_widget_designs();
		...
		<div class="bdpp-widget-content">
			...
			<div class="bdpp-widget-acc-cnt-wrap bdpp-widget-general <?php if( $instance['tab'] != 'general' ) { echo 'bdpp-hide'; } ?>">
				...
				<p>
					...
					<em><?php _e( 'Choose WordPress registered media size. e.g thumbnail, medium, large, full.', 'blog-designer-pack' ); ?></em>
				</p>
			...
			<div class="bdpp-widget-acc-cnt-wrap bdpp-widget-slider <?php if( $instance['tab'] != 'slider' ) { echo 'bdpp-hide'; } ?>">
				<!-- Arrows -->
				<p>
					<label for="<?php echo $this->get_field_id('arrows'); ?>"><?php _e( 'Arrows', 'blog-designer-pack' ); ?>:</label>
				...
				<!-- Dots -->
				<p>
					<label for="<?php echo $this->get_field_id('dots'); ?>"><?php _e( 'Dots', 'blog-designer-pack' ); ?>:</label>
					<select class="widefat" id="<?php echo $this->get_field_id( 'dots' ); ?>" name="<?php echo $this->get_field_name( 'dots' ); ?>">
						<option value="1" <?php selected( $instance['dots'], 1 ); ?>><?php _e('Yes', 'blog-designer-pack'); ?></option>
						<option value="0" <?php selected( $instance['dots'], 0 ); ?>><?php _e('No', 'blog-designer-pack'); ?></option>
					</select>
				</p>
				...
				<p>
					...
					<select class="widefat" id="<?php echo $this->get_field_id( 'autoplay' ); ?>" name="<?php echo $this->get_field_name( 'autoplay' ); ?>">
						<option value="1" <?php selected( $instance['autoplay'], 1 ); ?>><?php _e('Yes', 'blog-designer-pack'); ?></option>
						<option value="0" <?php selected( $instance['autoplay'], 0 ); ?>><?php _e('No', 'blog-designer-pack'); ?></option>
					</select>
				</p>

				<!-- Autoplay Interval -->
				<p>
					<label for="<?php echo $this->get_field_id( 'autoplay_interval' ); ?>"><?php _e( 'Autoplay Interval', 'blog-designer-pack'); ?>:</label>
					<input class="widefat" id="<?php echo $this->get_field_id( 'autoplay_interval' ); ?>" name="<?php echo $this->get_field_name( 'autoplay_interval' ); ?>" type="text" value="<?php echo bdpp_esc_attr( $instance['autoplay_interval'] ); ?>" />
				</p>

				<!-- Loop -->
				<p>
					<label for="<?php echo $this->get_field_id('loop'); ?>"><?php _e( 'Loop', 'blog-designer-pack' ); ?>:</label>
					<select class="widefat" id="<?php echo $this->get_field_id( 'loop' ); ?>" name="<?php echo $this->get_field_name( 'loop' ); ?>">
						<option value="1" <?php selected( $instance['loop'], 1 ); ?>><?php _e('Yes', 'blog-designer-pack'); ?></option>
						<option value="0" <?php selected( $instance['loop'], 0 ); ?>><?php _e('No', 'blog-designer-pack'); ?></option>
					</select>
				</p>

				<!-- Auto Height -->
				<p>
					<label for="<?php echo $this->get_field_id('auto_height'); ?>"><?php _e( 'Auto Height', 'blog-designer-pack' ); ?>:</label>
					<select class="widefat" id="<?php echo $this->get_field_id( 'auto_height' ); ?>" name="<?php echo $this->get_field_name( 'auto_height' ); ?>">
						<option value="1" <?php selected( $instance['auto_height'], 1 ); ?>><?php _e('Yes', 'blog-designer-pack'); ?></option>
						<option value="0" <?php selected( $instance['auto_height'], 0 ); ?>><?php _e('No', 'blog-designer-pack'); ?></option>
					</select>
				</p>
				...
				<p>
					...
					<select class="widefat" id="<?php echo $this->get_field_id( 'autoplay_hover_pause' ); ?>" name="<?php echo $this->get_field_name( 'autoplay_hover_pause' ); ?>">
				...
				<!-- Lazyload -->
				<p>
					<label for="<?php echo $this->get_field_id('lazyload'); ?>"><?php _e( 'Lazyload', 'blog-designer-pack' ); ?>:</label>
					<select class="widefat" id="<?php echo $this->get_field_id( 'lazyload' ); ?>" name="<?php echo $this->get_field_name( 'lazyload' ); ?>">
						<option value="1" <?php selected( $instance['lazyload'], 1 ); ?>><?php _e('Yes', 'blog-designer-pack'); ?></option>
						<option value="0" <?php selected( $instance['lazyload'], 0 ); ?>><?php _e('No', 'blog-designer-pack'); ?></option>
					</select>
				</p>
			</div><!-- end .bdpp-widget-acc-cnt-wrap -->
...
<?php
	...
	function widget( $widget_args, $instance ) {
		...
		$widget_designs			= bdpp_post_widget_designs();
		...
		$atts['slider_conf'] 	= compact('autoplay', 'autoplay_interval', 'arrows', 'dots', 'loop', 'speed', 'auto_height', 'autoplay_hover_pause', 'lazyload');
		...
		$args = apply_filters( 'bdpp_post_slider_widget_query_args', $args, $atts );
		...
		if( empty( $post_type ) ) {
			...
			echo __('Sorry, No Post Type is configured. Please enable form plugin settings and choose within widget.', 'blog-designer-pack');

		} elseif( $query->have_posts() ) {

			bdpp_generate_style( $style_id, 'widget', 'slider', 'bdpp-post-slider-widget', $atts );

			bdpp_get_template( 'widget/post-slider/loop-start.php', $atts );
			...
			while ( $query->have_posts() ) : $query->the_post();
				...
				// Include Dsign File
				bdpp_get_template( "widget/post-slider/{$design}.php", $atts );

			endwhile;

			bdpp_get_template( 'widget/post-slider/loop-end.php', $atts );
