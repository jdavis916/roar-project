...
class BDPP_Post_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Create the widget
		parent::__construct( 'bdpp-post-widget', __( 'BDPP - Post List Widget', 'blog-designer-pack' ), $widget_ops );

		// Widgets defaults
		$this->defaults = array(
				'title' 				=> esc_html__( 'Post List', 'blog-designer-pack' ),
...
<?php
	...
	function widget( $widget_args, $instance ) {
		...
		if( empty( $post_type ) ) {
			...
			bdpp_generate_style( $style_id, 'widget', 'grid', 'bdpp-post-widget', $atts );

			bdpp_get_template( 'widget/post-list/loop-start.php', $atts );
			...
			while ( $query->have_posts() ) : $query->the_post();
				...
				// Include Dsign File
				bdpp_get_template( "widget/post-list/{$design}.php", $atts );

			endwhile;

			bdpp_get_template( 'widget/post-list/loop-end.php', $atts );
