...
class BDPP_Shortcode_Generator {
	...
	function render( $args = array() ) {
		...
		if ( !empty($args) ) {
			...
			foreach ($args as $key => $value) {
				...
				echo '<div class="bdpp-shrt-acc-header">'.$section_title.'</div>';
				echo '<div class="bdpp-shrt-acc-cnt">';
				...
				foreach ($value['params'] as $param_key => $param_val) {
					...
					echo '<div class="bdpp-customizer-row" data-type="'.$param_val['type'].'">';
			...
			// Dependency Value
			if( $temp_dependency ) {
				echo '<div class="bdpp-cust-conf bdpp-cust-dependency" data-dependency="'.htmlspecialchars( json_encode( $temp_dependency ) ).'"></div>';
			}
		} else {
...
?>
		...
		<?php if( $args['heading'] ) { ?>
		<label class="bdpp-shrt-lbl" for="<?php echo $args['id']; ?>"><?php echo $args['heading']; ?></label>
		<?php } ?>
...
?>
		...
		<input type="text" id="<?php echo $args['id']; ?>" class="<?php echo $args['class']; ?>" name="<?php echo $args['name']; ?>" value="<?php echo $args['value']; ?>" data-default="<?php echo bdpp_esc_attr( $args['value'] ); ?>" <?php echo $allow_empty.' '.$refresh_time; ?> />
...
?>		
		<input type="number" id="<?php echo $args['id']; ?>" class="<?php echo $args['class']; ?>" name="<?php echo $args['name']; ?>" value="<?php echo $args['value']; ?>" step="<?php echo $step; ?>" min="<?php echo $min; ?>" max="<?php echo $max; ?>" data-default="<?php echo bdpp_esc_attr( $args['value'] ); ?>" <?php echo $refresh_time; ?> />
...
?>
		...
		<select id="<?php echo $args['id']; ?>" class="<?php echo $args['class']; ?>" name="<?php echo $args['name']; ?>" <?php echo (!empty( $args['multi'] )) ? 'multiple' : ''; ?> data-default="<?php echo bdpp_esc_attr( implode(',', $default) ); ?>" <?php echo $refresh_time; ?>>
...
<?php }
	...
	function render_field_radio( $args ) {
		...
		if( $args['value'] && is_array($args['value']) ) {
			foreach ($args['value'] as $select_key => $select_value) { ?>
				<label class="bdpp-shrt-field-lbl bdpp-cust-radio-lbl" for="<?php echo $args['id'].'-'.$select_key; ?>">
	...
	function render_field_checkbox( $args ) {
		...
		if( $args['value'] && is_array($args['value']) ) {
			foreach ($args['value'] as $select_key => $select_value) { ?>
				<label class="bdpp-shrt-field-lbl bdpp-cust-checkbox-lbl" for="<?php echo $args['id'].'-'.$select_key; ?>">
...
<?php
	...
	function render_field_colorpicker( $args ) { ?>
		...
		<input type="text" id="<?php echo $args['id']; ?>" class="bdpp-cust-color-box <?php echo $args['class']; ?>" name="<?php echo $args['name']; ?>" value="<?php echo $args['value']; ?>" data-default="<?php echo bdpp_esc_attr( $args['value'] ); ?>" />
